...
import { Gift, User, Plus, FileText, Loader2 } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#F5F7FA] font-sans pb-24">
        ...
        <div className="flex justify-between items-center relative z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white rounded-xl shadow-md flex justify-center items-center">
              ...
              <span className="w-2 h-2 bg-[#f97316] rounded-full absolute ml-4 mt-4" />
            </div>
            <h1 className="text-white font-bold text-lg tracking-wide">CS Partner App</h1>
          </div>
          <div onClick={() => navigate("/profile")} className="w-11 h-11 bg-white rounded-full flex items-center justify-center shadow-lg cursor-pointer">
      ...
      <div className="px-5 -mt-14 relative z-20">
        ...
        <div className="bg-white rounded-3xl p-6 flex justify-between items-center shadow-[0_8px_30px_rgb(0,0,0,0.06)] mb-6 border border-gray-100 relative overflow-hidden">
          <div className="absolute right-[-20px] top-[-20px] w-32 h-32 border-4 border-[#F0F4FF] rounded-full opacity-50 pointer-events-none" />
          <div className="z-10">
          ...
          <div className="w-16 h-16 bg-[#184F9E] rounded-full flex justify-center items-center shadow-lg relative z-10">
        ...
        <div className="bg-white rounded-[24px] p-6 shadow-sm mb-8 border border-gray-100 flex flex-col gap-5">
          <div className="flex gap-4 items-center">
            <div className="w-[70px] h-[90px] bg-[#E3EBFB] rounded-2xl p-[6px] shadow-inner flex-shrink-0">
          ...
          <div className="mb-8">
            ...
            <div className="flex overflow-x-auto gap-4 pb-4 -mx-1 px-1 snap-x no-scrollbar">
              {rewards.map((r) => {
                ...
                return (
                  <button key={r._id} onClick={() => navigate(`/rewards/${r._id}`, { state: { reward: r } })} className="bg-white rounded-[20px] p-4 min-w-[170px] border border-gray-100 shadow-sm flex flex-col items-center flex-shrink-0 snap-start active:scale-[0.98] transition-all">
            ...
            <div className="mt-2 flex justify-center">
              <button onClick={() => navigate("/rewards")} className="bg-[#1a4187] text-white text-sm font-bold py-3 px-8 rounded-full shadow-md w-[180px]">View All Gifts</button>
            </div>
          </div>
        )}
        ...
        <div className="mt-4 pt-6 border-t border-gray-100">
          <h3 className="font-bold text-gray-800 mb-4 px-1 flex items-center gap-2">
            <FileText size={18} className="text-[#1a4187]" /> Recent Bills
          </h3>
          <div className="space-y-3">
            {loadingBills && <div className="flex justify-center py-6"><Loader2 className="animate-spin text-slate-400" /></div>}
            ...
            {bills.map((b) => (
              <button key={b._id} onClick={() => navigate(`/bills/${b._id}`, { state: { bill: b } })} className="w-full bg-white rounded-[16px] p-4 border border-gray-100 shadow-sm hover:shadow-md transition flex items-center justify-between active:scale-[0.99]">
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  {b.pointsEarned > 0 && <span className="text-[11px] text-[#0f4089] font-bold">+{b.pointsEarned} pts</span>}
                </div>
              </button>
            ))}
          </div>
